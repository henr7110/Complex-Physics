# Repeat the above model with an external noise η > 0 selected from the distri-
# bution p(η) ∝ exp(−η2/(2·σ2)) with σ = 0.2. (At each time step: Replace all
# x < η with new random numbers ∈ [0; 1] and, in addition, select one random
# species j and set its xj to a new random number ∈ [0; 1]).
...
def Update(L,tumbler,a):
    counter = 0
    n = np.sqrt(-(sigma**2)*np.log(rdm.random()))
    if n > 0.9 and a > 1000:
        Shock = True
    else:
        Shock = False
...
ax3.set_yscale("log")
ax3.set_xscale("log")
ax3.hist(animate.tumbler,bins=500)
ax3.set_xlabel("Avalanche size")
ax3.set_ylabel("Count")
